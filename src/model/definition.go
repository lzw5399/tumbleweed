...
type ProcessDefinition struct {
	...
	Structure   json.RawMessage `gorm:"column:structure; type:jsonb" json:"structure" form:"structure"`                     // 流程的具体结构
	ClassifyId  int             `gorm:"column:classify_id; type:integer" json:"classifyId" form:"classifyId"`               // 分类ID
	Task        json.RawMessage `gorm:"column:task; type:jsonb" jsonb:"task" form:"task"`                                   // 任务ID, array, 可执行多个任务，可以当成通知任务，每个节点都会去执行
	SubmitCount int             `gorm:"column:submit_count; type:integer; default:0" json:"submitCount" form:"submitCount"` // 提交统计
	Notice      json.RawMessage `gorm:"column:notice; type:jsonb" json:"notice" form:"notice"`                              // 绑定通知
